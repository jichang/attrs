...
module OAuth =
    ...
    let oauth (code: string) (oauthCfg: OAuthConfig) =
        job {
            let body =
                {| code = code
            ...
            let request = Request.createUrl Post oauthCfg.ticketEndpoint |> Request.bodyString json
    ...
    let queryApps (url: string) (ticket: OAuthTicket) =
        job {
            let request =
